...
//TERA.inc
/************************************************************************************/
/*	Initialization process - 初期化処理																		*/
/************************************************************************************/
function InitLibrary() {
	//Restore hidden error messages - エラー非表示を元に戻す
	...
	CreatePageSetting();
	//UI-related function loading - UI系の関数ローディング
	Acodion();
	SmoothScroll();
	Hover();
	ScrollAction();
...
/************************************************************************************/
/*	Add class to links based on file extension - 拡張子に応じてリンクにクラスを追加												*/
/************************************************************************************/
function Extension() {
	$('a').each(function () {
...
/************************************************************************************/
/*	Scroll action - スクロール時の処理																*/
/************************************************************************************/
function ScrollAction() {
	if ($('body').hasClass('is-globalnav-open')) return;
	scroll_pos = 0;
	scroll_action_proc();
	...
	function scroll_action_proc(){
		...
		if ($('body').hasClass('is-globalnav-open')) {
      $('body').data('scroll_pos', scroll_pos); // Record only position - 位置の記録だけ
      return;
    }
...
/************************************************ */
//Vertical scroll action - 上下スクロール時の処理
/************************************************ */
function scroll_action(scroll_pos) {
	set_scroll_area_bottom();
	//Process based on scroll position - スクロール位置に応じた処理
...
function scroll_top_action() {
	//Process when scrolled to top - 上までスクロールしたときの処理
...
function scroll_bottom_action() {
	if (SCROLL_BOTTOM_ACTION == true) {
		//Process when scrolled to bottom - 下までスクロールしたときの処理
...
/************************************************ */
//Process on upward scroll - 上スクロール時の処理
/************************************************ */
...
/************************************************ */
//Process on downward scroll - 下スクロール時の処理
/************************************************ */
...
function Acodion() {
	//Setting
	let POINTER_SET = true; // Change cursor to pointer - カーソルをポインターにする？
...
/************************************************************************************/
/*	Smooth scroll - するするスクロール																*/
/************************************************************************************/
function SmoothScroll() {
	//Setting
	let SPEED = 300; //Scroll speed - スピード
	//Process on page transition - ページ遷移時の処理
	if (location.hash != '') {
		if ($(location.hash.length) != 0) {
			smooth_scroll_proc($(location.hash), SPEED);
		}
	}
	//Process on link click - リンククリック時の処理
...
/************************************************************************************/
/*	Smooth scroll execution - するするスクロール実行部														*/
/************************************************************************************/
function smooth_scroll_proc($target, speed) {
	...
	ScrollShift = 0; //Reset shift every time - シフトは毎回リセット
}

function Hover() {
	//Add hover class to <a> tags - aタグにhoverクラスをつける
...
/*																					*/
/*																					*/
/*	Subroutine part - サブルーチンパート																*/
/*																					*/
/*																					*/
/************************************************************************************/
/************************************************************************************/
/*	Get fixed header height - 固定ヘッダの高さを取得															*/
/************************************************************************************/
function get_fixed_header_height() {
	//Return 0 if header is not fixed - ヘッダが固定じゃなければ0を返す
...
/************************************************************************************/
/*	Get element height - 要素の高さを取得																*/
/************************************************************************************/
function get_element_height($elem) {
	return $elem.outerHeight();
}
/************************************************************************************/
/*	Get element top position - 要素のTOP位置を取得																*/
/************************************************************************************/
function get_element_top($target) {
	return $target.offset().top;
}
/************************************************************************************/
/*	Mobile detection - SP判定																			*/
/************************************************************************************/
function isSP() {
...
/************************************************************************************/
/*	Get device type - デバイス取得																	*/
/************************************************************************************/
function get_device_type() {
	var ret = DEVICE_PC_CLASS; //Desktop - PCファースト
...
/************************************************************************************/
/*	Set position at bottom of screen - 画面下部の位置を設定															*/
/************************************************************************************/
...
/******************************************************************************/
//	Function - 関数名：get_page_setting()
//	Process - 処理：Set page info - ページ情報の取得
//	Arguments - 引数：None - なし
//	Return - 戻り値：
/******************************************************************************/
...
//	処理：Get file name - ファイル名の取得
//	引数：rootpath(ファイル名を取得したいパス/空であれば現在のファイル)
//	戻り値：File name (excluding hash) ファイル名（ハッシュ含まず）
/******************************************************************************/
...
//	処理：Get hash from URL - URLの#以降を取得
//	引数：rootpath(ファイル名を取得したいパス/空であれば現在のファイル)
//	戻り値：ハッシュ
/******************************************************************************/
...
//	処理：Get location tree - locationTreeの取得
//	引数：None - なし
//	戻り値：Array [0] is root domain like http://***.com - 配列[0]がhttp://***.comなどルートドメイン
/******************************************************************************/
function get_location_tree(rootpath) {
	var temp = []; //Calculation array - 計算用配列
	var tree = []; //Return array - リターン用配列
	if (rootpath == undefined) {
		rootpath = window.location.pathname;
	} else {}
	temp = rootpath.split('/');
	//Get temporal array - 暫定配列の取得
	j = 0; //Calculation counter - 計算用カウンター
	for (i = 0; temp.length > i; i++) {
		if (i != 0) {
			//Ignore last layer the tree - 最終回層はツリーに含めない
			tree[j] = temp[i];
			j++;
		} else {
			//The first element in the array is the root domain - 配列の一つ目はルートドメイン
...
/******************************************************************************/
//	関数名：isTouchDevice()
//	処理：Detect touch device - タッチデバイスの判定
//	引数：None - なし
